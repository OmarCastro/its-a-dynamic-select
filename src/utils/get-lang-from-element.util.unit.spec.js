...
test('getLanguageFromElement should get correctly defined lang value', ({ dom }) => {
  const { document } = dom
...
test('getLanguageFromElement should ignore incorrectly defined html lang value', ({ dom }) => {
  // prepare
  const { document } = dom
  document.body.innerHTML = html`
        <div class="level-1" lang="pt">
...
test('getLanguageFromElement return navigation.language on undefined lang', ({ dom }) => {
  // prepare
...
test('getLanguageFromElement return navigation.language on invalid <html> lang', ({ dom }) => {
  // prepare
  const { document, navigator } = dom
...
test('getLanguageFromElement should get lang value on element inside shadow DOM', ({ dom }) => {
  // prepare
  const { document } = dom
